...
var sum = function (array) {
  // return the number if 1 number left
  if (array.length === 0) {
    return 0;
  } else {
...
var reverse = function (string) {
  ...
  return string.substring(string.length - 1, string.length) + reverse(string.substring(0, string.length - 1));
};
...
var countValuesInObj = function (obj, value) {
  ...
  for (let objKey in obj) {
    ...
    if (typeof obj[objKey] === 'object') {
      sum += countValuesInObj(obj[objKey], value);
    }
  }
...
var fibonacci = function (n) {
  ...
  let previousArr = fibonacci(n - 1);

  return previousArr.concat(previousArr[previousArr.length - 1] + previousArr[previousArr.length - 2]);
};
...
var capitalizeWords = function (array) {
  ...
  upperCasedArr.push(array[0].toUpperCase());

  return upperCasedArr.concat(capitalizeWords(array.slice(1)));
};
...
var capitalizeFirst = function (array) {
  ...
  capitalizedArr.push(array[0][0].toUpperCase() + array[0].slice(1));
...
var flatten = function (array) {
  ...
  for (let i = 0; i < array.length; i++) {
    ...
    if (Array.isArray(current)) {
      arr = arr.concat(flatten(current));
    } else {
      arr.push(current);
    }
  }
...
var compress = function (list) {
  ...
  return compressedArr.concat(compress(list.slice(1)));
};
